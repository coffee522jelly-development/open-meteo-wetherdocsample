  ...
  if( navigator.geolocation )
  {
    navigator.geolocation.getCurrentPosition(
      ...
      function( position )
      {
        ...
        // 風量
        const windurl = 'https://api.open-meteo.com/v1/forecast?latitude='+ lat +'&longitude='+ lng +'&hourly=windspeed_80m&timezone=Asia%2FTokyo'
        fetch(windurl)
        .then(data => data.json())
        .then(json => drawWindspeed(json))
          function drawWindspeed(json) {
            const mydata = {
              labels: json.hourly.time,
              datasets: 
              [{
                label: '風速',
                data: json.hourly.windspeed_80m,
                borderColor: 'orange',
                radius: 0
              }]
            }
